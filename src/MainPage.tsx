...
export default function MainPage() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (query) {
      navigate(`/news?query=${encodeURIComponent(query)}`); // 뉴스 페이지로 이동
    }
  };

  return (
    <div className="w-full min-h-screen bg-gray-50 flex flex-col items-center px-4 mt-12">

      {/* 🔍 검색창 섹션 */}
      <section className="w-full max-w-xl mb-20 text-center">
        <h3 className="text-xl font-semibold mb-4 text-gray-800 h-10 mt-15">
          🔎 뉴스 검색어를 입력하세요
        </h3>
        <p className="text-gray-600 mb-8 h-10">
